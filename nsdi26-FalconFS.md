...
* **热门文件名（Hot filenames）**：由于应用程序的命名惯例，特定文件名会频繁出现。例如，图6中的`map.json`；

* **哈希方差（Hash variance）**：如果唯一文件名的数量没有远多于元数据服务器的数量，文件名将会分布不均衡。
...
* **创建目录**
  * 客户端先根据混合元数据索引来计算`/b`的位置，然后向相应的MNode（`MNode 0`）发送`mkdir(/b)`请求。MNode 0执行路径解析，并在inode表中为`/b`创建inode，在本地命名空间副本中创建相应的dentry，并回应客户端。
  * 在mkdir执行成功后，MNode不会主动向其他MNode广播新的dentry。在执行路径解析时，如果发现了缺失的dentry，其他MNode会根据混合元数据索引来计算负责的MNode，并从中获取元数据信息。以`open(/b/c)`为例，在执行路径解析时，MNode 1发现`/b`的dentry缺失，随即向MNode 0发送查询请求，获取`/b`的元数据信息，并继续执行后续处理。

* **删除目录**
  * 客户端将`rmdir(/b)`请求发送给Coordinator。
  * Coordinator先获取所有祖先目录的共享锁和目标目录的互斥锁，然后将请求转发给该目录inode的所有者（`MNode 0`）。
  * MNode 0先对`/b`的inode上锁，阻塞后续的查询请求，并向其他MNode广播一个无效请求。
  * 收到请求后，每个MNode无效化本地的相应dentry，并搜索inode表，查询该目录的孩子，并向MNode 0返回是否存在孩子。
  * MNode 0聚合其他MNode的响应信息。若没有孩子，MNode 0会删除对应的inode，并通知Coordinator。而Coordinator则会释放所有的锁，并回应客户端。否则，MNode 0返回`-ENOTEMPTY`，终止rmdir操作。
...
代码仓库：[https://github.com/falcon-infra/falconfs](https://github.com/falcon-infra/falconfs)
